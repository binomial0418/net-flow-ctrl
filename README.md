...
```mermaid
timeline
    title 以預設 05:00 重置為例
    07/17 04:59 : 邏輯日 = 07/16 : 昨日額度仍在計算
    07/17 05:00 : 邏輯日 = 07/17 : ⟵ 重置，所有時數歸零
    07/17 12:00 : 邏輯日 = 07/17 : 正常累計
    07/17 23:59 : 邏輯日 = 07/17 : 跨午夜不重置
    07/18 04:59 : 邏輯日 = 07/17 : 仍算 07/17，額度不會回血
    07/18 05:00 : 邏輯日 = 07/18 : ⟵ 重置
```

已通過 host 測試驗證（見下方「測試」）。
...
編譯結果：Flash 約 77%（1,010 KB / 1,280 KB），RAM 約 15%。
...
* **設定頁沒有密碼保護**——任何連上熱點的人都能修改規則。若用於管理孩童上網，建議自行加上登入驗證。
