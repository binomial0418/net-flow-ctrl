...
| **登入保護** | 設定頁需帳密驗證，採 HTTP Digest（密碼不以明文傳輸） |
...
2. 開啟 **`http://192.168.4.1`**，以帳號 **`admin`** / 密碼 **`700418`** 登入
...
**登入驗證**：所有路由（含全部 API）皆需通過驗證，預設 `admin` / `700418`，定義在 `nfc_config.h`：
...
採用 **HTTP Digest** 而非 Basic：設定頁同時監聽在 STA（家用網路）介面上，Basic 會把密碼以 base64 明文形式隨每一次輪詢送上網路，Digest 送的則是雜湊。核心的 nonce 只在發出挑戰時重新產生、驗證成功後不會失效，因此網頁每 5 秒的輪詢會重用同一個 nonce，不會反覆跳出登入視窗。
...
* **登入密碼寫死在原始碼**（`nfc_config.h`），修改需重新編譯燒錄；也代表拿得到韌體的人就看得到密碼。
